...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        if self.config.get('humans', 'policy') == 'orca':
            self.case_capacity = {'train': np.iinfo(np.uint16).max - 2000, 'val': 1000, 'test': 1000}
            self.case_size = {'train': np.iinfo(np.uint16).max - 2000, 'val': config.getint('env', 'val_size'),
    ...
    def reset(self, phase='test', test_case=None):
        ...
        if self.config.get('humans', 'policy') == 'trajnet':
            ...
            self.robot.set(round(random.uniform(-1,1)*2), -self.circle_radius, round(random.uniform(-1,1)*2,2), self.circle_radius, 0, 0, np.pi / 2)
